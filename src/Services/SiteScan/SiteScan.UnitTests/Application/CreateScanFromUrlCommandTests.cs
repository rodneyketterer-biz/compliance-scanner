...
public class CreateScanFromUrlCommandTests
{
    ...
    public async Task Handler_PopulatesScanRecord_FromResolverResult()
    {
        ...
        Assert.AreEqual(result.OriginalSubmittedUrl, scan.OriginalSubmittedUrl);
        Assert.AreEqual(result.CanonicalScanRootUrl, scan.CanonicalScanRootUrl);
        Assert.AreSame(result.RedirectChain, scan.RedirectChain);
